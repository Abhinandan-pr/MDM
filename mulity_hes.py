...
# =====================================================================
# 1. Read AMISP name from environment, default to AMISP-1
TARGET_AMISP = os.getenv("AMISP_NAME")
# 2. Dynamically generate table prefixes (e.g., 'AMISP-2' becomes 'amisp2')
PREFIX = TARGET_AMISP.lower().replace("-", "")

# 3. Dynamic Table Names
TBL_COMMANDS = f"{PREFIX}_pending_commands"
TBL_STATE = f"{PREFIX}_consumer_edge_state"
TBL_SETTLEMENT = f"{PREFIX}_monthly_settlement"

logging.basicConfig(level=logging.INFO, format=f'%(asctime)s - [HES {TARGET_AMISP}] - %(message)s')
app = Flask(__name__)

POSTGRES_API_URL = os.getenv("POSTGRES_API_URL")
MDM_BULK_WEBHOOK_URL = os.getenv("MDM_BULK_WEBHOOK_URL")
MDM_PRIORITY_RC_WEBHOOK_URL = os.getenv("MDM_PRIORITY_RC_WEBHOOK_URL")
...
def init_db():
    with pg_engine.begin() as conn:
        ...
        """))
    logging.info(f"✅ PostgreSQL Initialized. {TARGET_AMISP} Isolation Guaranteed.")

init_db()

@app.route('/', methods=['GET'])
def health_check():
    return f"HES Edge Node [{TARGET_AMISP}] is Live. Tables isolated via prefix: {PREFIX}_", 200
...
def receive_commands_bulk():
    ...
    with pg_engine.begin() as conn:
        for i in range(0, len(insert_data), CHUNK_SIZE):
            ...
            conn.execute(text(f"""
                INSERT INTO {TBL_COMMANDS} (hes_tx_id, reference_id, meter_no, command) 
                VALUES (:hes_tx, :ref_id, :m_no, :cmd)
            """), chunk)
            
    logging.info(f"📥 Queued {len(insert_data)} Bulk Commands for {TARGET_AMISP}.")
    return jsonify({"status": "QUEUED", "count": len(insert_data)}), 202

@app.route('/internal/process-queue', methods=['GET'])
def process_queue():
    with pg_engine.connect() as conn:
        commands = conn.execute(text(f"""
    ...
    if not commands:
        return jsonify({"message": f"[{TARGET_AMISP}] RC-DC Queue empty."}), 200
    ...
    with pg_engine.begin() as conn:
        ...
        conn.execute(text(f"UPDATE {TBL_COMMANDS} SET status = 'COMPLETED' WHERE hes_tx_id IN ('{tx_list_str}')"))
        
        if meters_to_disconnect:
            dc_list_str = "', '".join(meters_to_disconnect)
            conn.execute(text(f"UPDATE {TBL_STATE} SET connection_status = 'D' WHERE meter_no IN ('{dc_list_str}')"))
            
        if meters_to_reconnect:
            rc_list_str = "', '".join(meters_to_reconnect)
            conn.execute(text(f"UPDATE {TBL_STATE} SET connection_status = 'C' WHERE meter_no IN ('{rc_list_str}')"))
...
def push_bulk_callbacks():
    ...
    return jsonify({"priority_rc_status": rc_response, "bulk_dc_status": dc_response}), 200
...
def push_lane_to_mdm(command_type, target_url):
    ...
    try:
        ...
        if resp.status_code in [200, 202]:
            ...
            with pg_engine.begin() as conn:
                conn.execute(text(f"UPDATE {TBL_COMMANDS} SET is_notified = TRUE WHERE hes_tx_id IN ('{tx_list_str}')"))
...
def generate_daily_reads():
    ...
    with pg_engine.connect() as conn:
        consumers = conn.execute(text(f"SELECT * FROM {TBL_STATE}")).mappings().fetchall()
    ...
    return jsonify({"message": f"Successfully generated reads for {len(consumers)} meters in {TARGET_AMISP}."}), 200
...
def export_daily():
    ...
    with pg_engine.connect() as conn:
        data = conn.execute(text(f"SELECT * FROM {TBL_STATE} LIMIT :l OFFSET :o"), {"l": limit, "o": offset}).mappings().fetchall()
